...
class PositionService
{
     ...
     * @param int $userId ID pengguna yang melakukan aksi.
     * @return Position
     * @throws DomainException Jika jabatan sudah dihapus atau merupakan cadangan sistem.
     */
    public function update(Position $position, array $data, int $userId)
    {
        // 1. Validate if the position can be updated
        if ($position->trashed()) {
            throw new \DomainException('Cannot update a deleted position');
        }
        if ($position->system_reserve) {
            throw new \DomainException('Cannot update a system reserve position');
        }
     ...
     * @return bool
     * @throws DomainException Jika jabatan sudah dalam status terhapus atau merupakan cadangan sistem.
     */
    public function delete(Position $position): bool
    {
        // 1. Security and state validation
        if ($position->trashed()) {
            throw new \DomainException('Cannot delete a deleted position');
        }
        if ($position->system_reserve) {
            throw new \DomainException('Cannot delete a system reserve position');
        }
     ...
     * @return Position
     * @throws DomainException Jika jabatan tidak dalam status terhapus.
     */
    public function restore(string $uuid): Position
    {
        return DB::transaction(function () use ($uuid) {
            ...
            if (! $position->trashed()) {
                throw new \DomainException('Position is not deleted');
            }
     ...
     * @return void
     * @throws DomainException Jika UUID tunjangan tidak ditemukan.
     */
    private function syncAllowances(Position $position, array $data): void
    {
        ...
        foreach ($data['allowances'] as $item) {
            ...
            if (! isset($allowances[$item['uuid']])) {
                throw new \DomainException("Allowance UUID {$item['uuid']} not found");
            }
